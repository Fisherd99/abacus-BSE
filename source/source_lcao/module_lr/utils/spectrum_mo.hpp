                        ...
                        const K_Vectors& kv)
{
    ...
    ofs << "Data Unit (Ry * Bohr)    " << filename << std::endl;
    ...
    for (int is = 0; is < nspin_tmp; ++is)
    {
        ...
        for (int ik = 0;ik < nk;++ik)
        {
            ...
            ofs << std::setw(4) << "KS1" << std::setw(8) << "E1(eV)" << std::setw(6) << "KS2" << std::setw(8) << "E2(eV)"
            ...
            for (int i = 0; i < KS_num; ++i)
            {
                for (int j = i; j < KS_num ; ++j)
                {
                    ...
                    ofs << std::setw(3) << i << std::setw(10) << std::setprecision(6) << eig_ks[i + ik*KS_num] * ModuleBase::Ry_to_eV
                    << std::setw(4) << j << std::setw(10) << eig_ks[j + ik*KS_num] * ModuleBase::Ry_to_eV
                        ...
                        const K_Vectors& kv)
{
    ...
    double BohrRyToAngEv = ModuleBase::Ry_to_eV * ModuleBase::BOHR_TO_A; // 13.605698 * 0.5291770
    for (int is = 0; is < nspin_tmp; ++is)
    {
        for (int ik = 0; ik < nk; ++ik)
        {
            for (int id = 0; id < 3; ++id)
            {
                ...
                for (int ib1 = 0; ib1 < KS_num; ++ib1)
                {
                    for (int ib2 = 0; ib2 < KS_num; ++ib2)
                    {
                        ...
                        ofs << std::setw(26) << std::fixed << std::setprecision(16) << out_spectrum_mo[ipair].real() * BohrRyToAngEv
                            << std::setw(26) << out_spectrum_mo[ipair].imag() * BohrRyToAngEv << std::endl;
                    }
                }
            }
