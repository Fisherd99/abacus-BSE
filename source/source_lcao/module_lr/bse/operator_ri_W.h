...
{
    template<typename T>
    class OperatorRIW : public hamilt::Operator<T>
    {
        using TR = T;
    public:
        OperatorRIW(const UnitCell& ucell,
            const int& naos,
            const int& nocc,
            const int& nvirt,
            ...
            CV_vo(cal_CV(Cs_vo_mo, Ws)),
            CV_ov(cal_CV(Cs_ov_mo, Ws))
        {
            ModuleBase::TITLE("OperatorRIW", "OperatorRIW");
            this->cal_type = hamilt::calculation_type::lcao_gint;
            this->act_type = 2;
            this->is_first_node = true;
            // check: compare with CWC
            std::vector<T> Amat1 = cal_Amat_full(Cs_vo_mo, Cs_vo_mo, Ws);
